                    ...
                    child: Column(
                      children: [
                        ...
                        Transform.translate(
                          ...
                          child: Column(
                            children: [
                              Text(user.displayName, style: heading(23, weight: FontWeight.w900)),
                              const SizedBox(height: 3),
                              Text(user.username, style: body(13, color: AppColors.muted, weight: FontWeight.w700)),
                              const SizedBox(height: 16),
                              InkWell(
                                onTap: () async {
                                  final following = await _social.toggleFollow(user.uid);
                                  if (!mounted) return;
                                  if (following == null) return;
                                  showLumoraToast(
                                    this.context,
                                    following ? 'Vous suivez maintenant ${user.displayName} ✓' : 'Vous ne suivez plus ${user.displayName}',
                                  );
                                },
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    borderRadius: BorderRadius.circular(AppRadius.button),
                                    border: AppBorders.neo(width: 2),
                                    boxShadow: isFollowing ? null : AppShadows.neo(offset: 2.5),
                                  ),
                                  child: Text(
                                    isFollowing ? 'Abonné ✓' : 'Suivre +',
                                    style: body(13, weight: FontWeight.w900, color: isFollowing ? AppColors.text : AppColors.surface),
                                  ),
                                ),
                              ),
                              const SizedBox(height: 18),
                              Text(user.bio, style: body(13, height: 1.45), textAlign: TextAlign.center),
                              const SizedBox(height: 24),
                              Row(
                                children: [
                                  _stat('${user.followersCount}', 'Abonnés'),
                                  const SizedBox(width: 10),
                                  _stat('${user.followingCount}', 'Abonnements'),
                                  const SizedBox(width: 10),
                              ...
                              Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Text('Série actuelle : ${user.streak} jours', style: body(13, weight: FontWeight.w900)),
                                  ],
                                ),
                              ),
                            ],
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 3),
            Text(label, style: body(9.5, color: AppColors.muted, weight: FontWeight.w700), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
